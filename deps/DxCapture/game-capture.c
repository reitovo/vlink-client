...
static inline void get_config(struct game_capture_config *cfg,
                              struct dx_capture_setting_t *settings) {
    const char *mode_str = NULL;

    ms_build_window_strings(settings->window, &cfg->class, &cfg->title,
                            &cfg->executable);
...
void game_capture_update(void *data, struct dx_capture_setting_t *settings) {
    ...
    gc->target_fps = settings->target_fps;
    gc->error_acquiring = false;
    gc->activate_hook = !!settings->window && !!*settings->window;
...
void *game_capture_create(struct dx_capture_t* cap) {
    ...
    game_capture_update(gc, &cap->setting);
    return gc;
}
...
static inline void reset_frame_interval(struct game_capture *gc) {
    uint64_t interval =
            util_mul_div64(gc->target_fps.den, 1000000000ULL, gc->target_fps.num);
...
static inline bool inject_hook(struct game_capture *gc) {
    ...
    inject_path = dx_module_file(gc->cap, "inject-helper.exe");
    hook_path = dx_module_file(gc->cap, "graphics-hook.dll");
    ...
    if (!matching_architecture) {
        warn("process architecture mismatch!");
        return false;
    }
...
enum capture_result {
    CAPTURE_FAIL, CAPTURE_RETRY, CAPTURE_SUCCESS
};
...
static void copy_shmem_tex(struct game_capture *gc) {
    ...
    if (dx_texture_map(gc->cap, gc->texture, &data, &pitch)) {
        if (gc->convert_16bit) {
        ...
        dx_texture_unmap(gc->cap, gc->texture);
    }
